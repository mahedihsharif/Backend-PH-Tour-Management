...
const getUserStats = async () => {
  ...
  const usersByRolePromise = User.aggregate([
    //state-1: Grouping users by role and count total users in each role
...
const getTourStats = async () => {
  ...
  const totalTourByDivisionPromise = Tour.aggregate([
    ...
    //stage-3: grouping tour division
    {
      $group: {
        _id: "$division.name",
        count: { $sum: 1 },
  ...
  return {
    totalTour,
    totalTourByTourType,
    avgTourCost: avgTourCost[0].avgCostFrom,
    totalTourByDivision,
    totalHighestBookedTour,
  };
};
